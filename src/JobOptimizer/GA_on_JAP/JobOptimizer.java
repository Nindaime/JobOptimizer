...
import JobOptimizer.GUI_Module.FitnessChartController;
import JobOptimizer.GUI_Module.DirPort;
import JobOptimizer.GUI_Module.ScreenController;
import JobOptimizer.Graph.GraphInit;
import JobOptimizer.Graph.WeightedEdge;
import JobOptimizer.Graph.UCSDIRPort;
import JobOptimizer.Graph.Vertex;
...
import static JobOptimizer.GUI_Module.FXMLDocumentController.FitnessChart;
import static JobOptimizer.GUI_Module.FXMLDocumentController.displaySubMenu;

/**
 *
 * @author PETER-PC
 */
public class JobOptimizer implements Comparable{
    private TrainSet[] trainset;
    public static final int TRIPCOUNTPERDAY = 18, INDIVIDUAL = 5;
    public final static ObservableList<JobOptimizer> TCPs = FXCollections.observableArrayList();
...
//sample project with similar concepts whose implentations can inspire your work
    public JobOptimizer(TrainSet[] t){
    ...
    public JobOptimizer(TrainSet[] t, String s){
...
//
    ...
    public synchronized void updateProgress(double addedProgress, String pBarLabelText){
        AnchorPane mainPageRoot = (AnchorPane) ScreenController.screens.get(JobOptimizer.JobOptimizer.mainPage);
    ...
    public static class BuildStopPoints extends RecursiveAction{
        ObservableList<JobOptimizer> tcpList;
        
        public BuildStopPoints(ObservableList<JobOptimizer> tcpList){
        ...
        protected void compute(){
            if(tcpList.size() == 1){
                JobOptimizer currentTCP = tcpList.get(0);
                float progress = 0;
                for (int i = 0; i < 6; i++) {
                    for (int k = 0; k < TRIPCOUNTPERDAY; k++) {
                ...
                ObservableList<JobOptimizer> firstArrayList = FXCollections.observableArrayList(tcpList.subList(0, tcpList.size()/2));
                ObservableList<JobOptimizer> secondArrayList = FXCollections.observableArrayList(tcpList.subList(tcpList.size()/2, tcpList.size()));
...
//        }catch(SQLException ex){}
    ...
    public static synchronized void simulationTCP(JobOptimizer simulatedTCP, boolean play){
...
//        JobOptimizer simulatedTCP = TCPs.get(tcpIndex);
        TableView tbView = (TableView) container.lookup("#tbView");
...
//                        if (canPlay) {
                            ...
                            gBests.add((JobOptimizer)Collections.max(TCPs));
...
//        JobOptimizer tcp = TCPs.get(tcpIndex);
    ...
    public static void playCurrentAnimation(AnchorPane mainPageRoot, AnchorPane container){
        JobOptimizer animatedTCP = gBests.get(gBests.size()-1);
...
//        double progressFraction = 1 / (getMaxDuration(TCPs.indexOf(animatedTCP)) * 0.1);
    ...
    public static synchronized void clearCurrentAnimation(AnchorPane mainPageRoot, AnchorPane container, JobOptimizer currentTCP){
    ...
    public static ArrayList<JobOptimizer> gBests = new ArrayList<>();
...
//            }
            ...
            ArrayList<JobOptimizer> genePool = new ArrayList<>();
            for(JobOptimizer t: TCPs){
...
//                        System.out.println("sleep thread");
            ...
            ArrayList<JobOptimizer> childrenTCP = new ArrayList<>();
...
//                JobOptimizer childTCP = new JobOptimizer(genePool.get(selectedParentIndex1).getTrainSet(), "optimization");
//
...
//            for(JobOptimizer t: childrenTCP){
//                
...
//            });
    ...
    public static class mutateTCPs extends RecursiveAction{
        private final ArrayList<JobOptimizer> childrenTCP;
        private final int mutationPosibility;
        
        public mutateTCPs(ArrayList<JobOptimizer> childrenTCP, int mutationPosibility){
        ...
        protected void compute(){
            if(childrenTCP.size() == 1) {
                ...
                ArrayList<JobOptimizer> firstChildList = new ArrayList<>();
                ArrayList<JobOptimizer> secondChildList = new ArrayList<>();
    ...
    public static class crossoverTCPs extends RecursiveAction{
        ...
        private final ArrayList<JobOptimizer> genePool;
        private final ArrayList<JobOptimizer> childrenTCP;
        
        public crossoverTCPs(int crossoverPossibility, ArrayList<JobOptimizer> genePool, ArrayList<JobOptimizer> childrenTCP){
        ...
        protected void compute(){
            if (crossoverPossibility == 1) {
                ...
                JobOptimizer childTCP;
                synchronized(genePool){
                    childTCP = new JobOptimizer(genePool.get(selectedParentIndex1).getTrainSet(), "optimization");
                }
...
//        ResultSet rSet = statement.executeQuery();
        ...
        for(int j = 0; j < i; j++) {
            ...
            new JobOptimizer(tSArray);
        }
    }
    
    
    public boolean shouldRepair(Station s, JobOptimizer t, int rowIndex, int columnIndex){
        Station s1, s2;
...
//    }
    ...
    public final void repairTrainSet_V2(Station s, JobOptimizer t, int rowIndex, int columnIndex){
//        int tcpIndex = (individualIndex + 1) * (rowIndex + 1) - 1;
        synchronized(TCPs){
    ...
    public final boolean isValidStationSlot(Station station, JobOptimizer t, int rowIndex, int columnIndex){
    ...
    public final Station resolveIsValidStationSlot(Station s, JobOptimizer t, int rowIndex, int columnIndex){
...
//    }
    ...
    public int compareTo(Object o) {
        if(o instanceof JobOptimizer){
            double fitnessValue1 = this.getFitness().get("passengerSatisfaction") + this.getFitness().get("tcpTrainUtilizationEfficiency");
            double fitnessValue2 = ((JobOptimizer) o).getFitness().get("passengerSatisfaction") + ((JobOptimizer) o).getFitness().get("tcpTrainUtilizationEfficiency");
